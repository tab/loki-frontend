...
import { PHONE_NUMBER_LENGTH, PHONE_NUMBER_REGEX } from "@/helpers/phone_number"

const LOCALES = ["ENG", "EST"]

export default function MobileIdLogin() {
  ...
  const handleValidate = (values: { locale: string; personal_code: string; phone_number: string }) => {
  ...
  const handleSubmit = async (values: { personal_code: string; locale: string }, setSubmitting: (isSubmitting: boolean) => void) => {
    setErrorMessage(null)

    try {
      const response = await fetch(`/api/auth/mobile_id`, {
        method: "POST",
        headers: {
  ...
  return (
    <div>
      <h5>{i18n.t("pages.login.providers.mobile_id")}</h5>
          ...
          {({ isSubmitting }) => (
            <Form className={styles.form}>
              ...
              <Input
                type="text"
                name="phone_number"
                placeholder={i18n.t("phone_number.form.placeholder")}
                autoComplete="off"
              />
              <Input
                type="text"

...
export default function NewPermissionPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (values: FormValues) => {
      const { data } = await axiosInstance.post("/backoffice/permissions", values)
      return data
    },
    onSuccess: () => {
      // @ts-ignore
      queryClient.invalidateQueries(["permissions"])
      enqueueSnackbar(i18n.t("permission.snackbar.create.success"), { variant: "default" })
      router.push("/backoffice/permissions")
    },
    onError: error => {
      enqueueSnackbar(i18n.t("permission.snackbar.create.error"), { variant: "default" })
      console.error(error)
    },
  })
  ...
  return (
    <>
      ...
      >
        <Title>
          {i18n.t("pages.backoffice.permissions.new.title")}
        </Title>
      </Box>

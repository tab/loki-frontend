...
export default function EditScopePage() {
  ...
  const { data, isLoading, isError } = useQuery<ScopeType>({
    queryKey: ["scope", id],
    queryFn: async () => {
      const { data } = await axiosInstance.get(`/backoffice/scopes/${id}`)
      return data
    },
    enabled: !!id,
  })
  ...
  const updateMutation = useMutation({
    mutationFn: async (values: FormValues) => {
      const { data } = await axiosInstance.put(`/backoffice/scopes/${id}`, values)
      return data
    },
    onSuccess: () => {
      // @ts-ignore
      queryClient.invalidateQueries(["scopes"])
      enqueueSnackbar(i18n.t("scope.snackbar.update.success"), { variant: "default" })
      router.push("/backoffice/scopes")
    },
    onError: error => {
      enqueueSnackbar(i18n.t("scope.snackbar.update.error"), { variant: "default" })
      console.error(error)
    },
  })
  ...
  return (
    <>
      ...
      >
        <Title>
          {i18n.t("pages.backoffice.scopes.edit.title", { name: data.name })}
        </Title>
      </Box>
